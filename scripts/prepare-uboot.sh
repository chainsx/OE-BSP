...
if [[ -f $workdir/config/u-boot/$BOARD.dl ]];then
  source $workdir/config/u-boot/$BOARD.dl
  ...
  if [[ -f $workdir/config/u-boot/$BOARD.build ]];then
    bash $workdir/config/u-boot/$BOARD.build  # $BOARD.build will build u-boot and generate idb and itb file at $build_dir/u-boot
